...
class InputExample(object):
    ...
    def __init__(self, guid, evidence, claim=None, label=None):
        """Constructs a InputExample.

        Args:
            guid: Unique id for the example.
            evidence: string. The untokenized text of the first sequence. For single
...
class FeverProcessor(DataProcessor):
    ...
    def get_train_pos_examples(self, data_dir):
        ...
        return self._create_examples_train(
            self._read_json(os.path.join(data_dir, "train_pos_sentences.json")), "train")

    def get_train_neg_examples(self, data_dir):
        """See base class."""
        return self._create_examples_train(
            self._read_json(os.path.join(data_dir, "train_neg_sentences.json")), "train")
...
def compute_metrics(task_name, preds, labels,guid1_batch,guid2_batch,athene_labels):
    ...
    if task_name == "cola":
        ...
        return {"acc_Aggregate": aggregate_acc,
                "acc_perSample": accuracy_score(preds, labels),
                "acc_Fever": fever_acc,
...
def main():
    ...
    # config = RobertaConfig.from_pretrained(args.bert_model,from_tf=False,num_labels=num_labels,output_hidden_states=False)
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if output_mode == "classification":
                    ...
                    if args.losstype == 'cross_entropy':
                        ...
                        loss = loss_fct(output_logits.logits.view(-1, num_labels),label_ids_cat)
                        Neg_index = label_ids_cat.view(-1) == 2
                        Pos_index = (label_ids_cat.view(-1) == 0) | (label_ids_cat.view(-1) ==1)
                        ...
                        IDS = torch.cat([torch.tensor(range(0,Pos_numbers),device=device), Pos_numbers+Hard_Neg_index], dim=0).to(torch.int32)
                        # output_logits = model(input_ids_cat[IDS, :], segment_ids_cat[IDS, :], input_mask_cat[IDS, :], labels=None)
                        ...
                        # acc, f1, acc_and_f1 = acc_and_f1(pred.cpu().numpy(), label_ids_cat[IDS].cpu().numpy())
    ...
    model.to(device)
        ...
        # pickle_in.close()
        # pickle_in = open(args.output_dir + 'predict.p', 'wb')
        # pickle.dump(preds,pickle_in)
        # pickle_in.close()
