...
class FeverProcessor(DataProcessor):
    ...
    def get_train_pos_examples(self, data_dir):
        ...
        return self._create_examples_train(
            self._read_json(os.path.join(data_dir, "train_evidence.json")), "train")

    def get_train_neg_examples(self, data_dir):
        """See base class."""
        return self._create_examples_train(
            self._read_json(os.path.join(data_dir, "train_non_evidence.json")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples_dev(
            self._read_json(os.path.join(data_dir, "dev_classifier_evidence.json")),
            "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples_dev(
            self._read_json(os.path.join(data_dir, "test_evidence.json")),
            "test")
    ...
    def _create_examples_test(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            claim = line[4]
            examples.append(
                InputExample_dev(guid=guid, claim_id=claim_id, num_sentence=num_sentence, evidence=evidence))
        return examples
...
def convert_examples_to_features_test(examples,
                                      label_list,
                                      max_seq_length,
                                      tokenizer,
                                      output_mode):
    ...
    for (ex_index, example) in enumerate(tqdm(examples, desc="Writing Example Eval")):
        ...
        if ex_index < 1:
            ...
            print()
        features.append(
            InputFeatures_test(input_ids=input_ids,
                                input_mask=input_mask,
                                segment_ids=segment_ids,
                                guid=example.guid,
                                claim_id=example.claim_id,
